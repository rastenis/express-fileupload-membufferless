...
function processMultipart(options, req, res, next) {
    ...
    busboy.on('file', function(fieldname, file, filename, encoding, mime) {
        ...
        let randomBufferName = shortid.generate();
        let bufferFile = fs.createWriteStream("static/tmp/" + randomBufferName);
        let size = 0;
        ...
        file.on('limit', () => {
            ...
            // removing the temp file if created already
            try {
                fs.remove("static/tmp/" + randomBufferName, err => {
        ...
        file.on('end', function() {
            ...
            let newFile = {
                name: filename,
                data: null, // already stored & can't process large files
                dataSize: size,
                encoding: encoding,
                truncated: file.truncated,
                mimetype: mime,
                // md5: md5(buf), inaccessible as it stands
                mv: function(path, callback) {
                    ...
                    function doMove(successFunc, errorFunc) {
                        fs.move("static/tmp/" + randomBufferName, path, function(err) {
                            if (err) {
                                console.log(err);
                                errorFunc(err);
                            } else {
